...
- If you created a system service for your node by following the instructions on the previous page, [Set Up a Full Node](./set-up-a-full-node.mdx), start your node with the following command:
  ```bash
...
Request a list of healthy peers for your deployment from a [Live Peer Node](./resources.mdx#live-peer-node-providers) provider.
...
Download an up-to-date `addrbook.json` file for your deployment from an [Address Book](./resources.mdx#address-book-providers) provider. 
...
To use state sync for quick recovery in case your node falls out of sync, follow the instructions for your deployment from a [State Sync](./resources.mdx#state-sync-service) service.
...
To save a snapshot on your system for quick recovery in case your node falls out of sync, install a snapshot for your deployment from a [Snapshot Service](./resources.mdx#snapshot-service).
